...
В проекте есть автоматические тесты на **pytest**:

**Что покрыто**
- **API (интеграционные тесты):**
  - `POST /links-tracker/visited_links` — нормализация ссылок, сохранение доменов, устранение дублей.
  - `GET /links-tracker/visited_domains` — выборка уникальных доменов по интервалу времени.
  - Обработка ошибок: `400` при `from > to`, `422` при некорректном теле запроса.
- **Бизнес-логика (юнит-тесты):**
  - `LinkService.extract_domain` — извлечение домена из разных вариантов URL.
  - `LinkService.add_visited_links` — нормализация + уникальность доменов.
- **Инфраструктура в тестах:**
  - In-memory репозиторий вместо Redis.
  - Dependency Override для подмены `LinkService` в FastAPI.
